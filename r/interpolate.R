...
#' Returns a data.frame with all diameters and years, including interpolated
#' values (if any).
#'
...
interpolate <- function(diam, years, all_years) {
  ...
  df <- data.frame(diam, year = years) |>
    ...
    # remove values before the first of after the last measured diameter
    subset(year >= min(years) & year <= max(years))
