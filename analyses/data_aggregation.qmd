...
```{r}
gr_range <- quantile(data_cor$diam_growth, c(0.01, 0.99), na.rm = TRUE)
subset(
  data_cor,
  diam_growth >= gr_range[1] & diam_growth <= gr_range[2]
) |>
  ggplot(aes(x = diam_growth)) +
  geom_histogram(aes(fill = factor(Plot)), binwidth = 0.1) +
  labs(x = "Diameter growth (cm/yr)", fill = "Plot") +
  theme_classic()
```

This graph shows all DBH growth values (in cm/yr) between the first and the 99th percentiles, i.e. between `r gr_range[1]` and `r gr_range[2]` cm/yr. All values of DBH growth outside this range are considered outliers. 
...
Outliers in DBH growth (but not DBHs) are replaced by mean DBH growth. DBH growth values are considered outliers when their value is less than the 1st percentile of DBH growth values (`r gr_range[1]`) or greater than the 99th percentile (`r gr_range[2]`).

Note: 

- Should we define thresholds and/or values to be replaced by group? size class, species groups? limitation: certain size classes (e.g. large trees) or other groups could be more prone to errors, which could create biases. 
...
```{r}
average_growth <- data_cor |>
  subset(diam_growth >= gr_range[1] & diam_growth <= gr_range[2]) |>
  with(mean(diam_growth, na.rm = TRUE))

data_cor <- data_cor |>
  mutate(diam_growth_cor = ifelse(
    !is.na(diam_growth) &
      (diam_growth < gr_range[1] | diam_growth > gr_range[2]),
    average_growth,
    diam_growth
...
id_cor <-
  subset(
    data_cor,
    diam_growth_cor != diam_growth & !is.na(diam_growth_cor)
  )$IdTree
tree_subs <-
  data_cor |>
...
data_cor |>
  ...
  mutate(subs = diam_growth_cor != diam_growth & !is.na(diam_growth_cor)) |>
  pivot_longer(cols = c("diam_growth", "diameter_cor")) |>
  ggplot(aes(x = Year)) +
  geom_point(aes(y = value, col = subs)) +
...
data_cor$agb <- with(
  data_cor,
  computeAGB(D = diameter_cor, WD = wd, coord = c(11, 0))
)
data_cor <- data_cor |>
  group_by(IdTree) |>
  mutate(diff_agb = c(NA, diff(agb)) / diff_year)
```
...
data_cor <- data_cor |>
  merge(data_year) |>
  group_by(IdTree) |>
  mutate(recr_year = min(Year), mort_year = max(Year)) |>
  ungroup() |>
  mutate(
    first_census = min(Year),
    status = ifelse(Year == recr_year & Year != first_census, "R", "A")
  )
```

```{r recruitment}
data_aggr <- data_cor |>
  subset(Year == recr_year & Year != first_census) |>
  group_by(Plot, Subplot, Year) |>
  summarise(
    agb_recr = sum(agb / (Year - prev_census) / SubplotArea, na.rm = TRUE)
  ) |>
  merge(data_aggr)
...
```{r growth}
data_aggr <- data_cor |>
  subset(Year != recr_year) |>
  group_by(Plot, Subplot, Year) |>
  summarise(agb_growth = sum(diff_agb / SubplotArea)) |>
  merge(data_aggr)
