...
if (all(data_cor$SpName_codecor == "no_cor")) { # no duplicated names
  text <- paste(
    ...
    "is the same than in scientific_cleaned, and SpName_codecor is always 'no_cor'"
  )
...
data_cor <- data_cor |>
  ...
  ungroup() 
    ...
    #mutate(
  #diam_growth = c((diameter_cor[2] - diameter_cor[1]) / (Year[2] -   #Year[1]), diff(diameter_cor) / diff(Year)),
  #diff_year = c(Year[2] - Year[1], diff(Year)))
