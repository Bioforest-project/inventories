...
googledrive::drive_get(as_id("1fq2owxMBLBwwibcdw2uQQFxnIhsMbaH4Qcj_xUwVvSQ")) |>
...
plot_information <- plot_information |>
  ...
  select(site, plot, longitude, latitude, logging, silv_treat)
...
The following plots do not have information on either their coordinates or their treatment and have been removed from the analysis: `r paste(subset(data_missing, is.na(longitude))$idplot, collapse = ", ")`. Please add their information to the online document or check that the site and plot names are consistent between your harmonized data and the online document.   

# General presentation of the data

This document presents the data aggregation for the `r paste(unique(data_raw$Site), collapse = ", ")` site(s). This site had `r length(unique(data_raw$Plot))` plots: `r unique(data_raw$Plot)`.
...
-   "gapfilled" observations with NA in the ScientificName, but for which other observations of the same trees have a unique ScientificName. These case were gapfilled but putting the scientific name of the other observations in ScientificName_cor
...
> To discuss : it would be worth it gapfilling the scientific name when some obs are indet. as well, But I don't know if the Inded are always coded the same, and how...
...
species_count <- data_cor %>%
  group_by(Site, Plot, Year, ScientificName) %>%
...
neutral_div <- species_count %>%
  ...
  summarise(
    ...
    evenness = as.numeric(Shannon(count)) / log(n_distinct(ScientificName))
  )
...
div_plot <-
  ggplot(results_long, aes(x = Year, y = value, color = as.factor(Plot))) +
  geom_line() +
