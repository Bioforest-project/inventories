...
This document presents the data aggregation for the `r paste(unique(data_cor$Site), collapse = ", ")` site(s). The site(s) have `r length(unique(data_cor$Plot))` plots.
...
if (params$site %in% c("paracou", "montagne-tortue", "bafog", "manare")) {
  vern <- data_cor |> 
    ...
    summarise(scientific_vern = names(
      sort(table(scientific_cleaned),decreasing=TRUE))[1])|> 
  ...
  text <- paste("For sites in French Guiana, where there is information on the", 
                "vernacular name but not on the scientific name, we add the scientific name", 
                "that was most frequently associated to that vernacular name in the rest", 
                "of the dataset.")
} else {text <- "If your site has only vernacular names, and if you have any information for the corresponding scientific names, please contact us to include the table in our analysis."}
```
...
if (any(data_cor$Site == "Iwokrama")) {
  ...
  text <- paste("In Iwokrama, we have removed census year information from the",
...
if (all(length(unique(nb_stem_plot$n)) == 1, unique(nb_stem_plot$n) == 1)) {
  text <- "There is no duplicated stem identifiers within a same census in this site, so we move on to checking for multi-stemmed trees."
} else {
...
if (length(unique(nb_stem_plot$n)) != 1) { # if there are some duplicates
  ...
  if (prev_nb_obs - dim(data_cor)[1] != 0) {
    ...
    text <- "We checked for true duplicates (fully identical rows), and there are none in this dataset."
  }
...
if (still_stem_dupli == TRUE) {
  ...
  text <-  "There are no more duplicates measurements in this dataset. We will now move on to checking for multi-stemmed trees."
}
```
...
The DBH values in this dataset are thus NA and zero free.
...
```
As mentioned above, to maintain consistency, we will remove all stems below 10 am, if there are any.
...
if (any(!is.na(species_groups$med_growth))) {
  species_groups |> 
    ...
    group_by(sp_group) |> 
    summarise(range = paste(paste(round(range(med_growth), 2), collapse = " - "), "cm/yr"), 
              species = paste(scientific_cleaned, collapse = ", ")) |> 
    select(range,	species) |> t() |>  
...
if (sum(data_cor$outlier) > 0) {
  text <- paste("This graph shows all outlier DBH growth values (in cm/yr),", 
  ...
  text <- paste("There were no diameter growth values to correct, as no stem",
...
if (sum(data_cor$outlier) > 0) {
  ...
  labs_diam <- c(diam_growth = "Diameter growth rate (cm/yr)", 
  ...
  data_cor |>
    ...
    scale_color_manual(values = c("black", "red")) +
    facet_grid(name ~ label, scales = "free", switch = "y",
