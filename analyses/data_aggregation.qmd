...
if (single_site) {
  ...
  # data_raw <-  # nolint
  #   list.files("data/raw_data/", "paracou", full.names = TRUE) |>  # nolint
  #   lapply(read.csv) |>  # nolint
  #   rbindlist(fill = TRUE, use.names = TRUE) |>  # nolint
  #   as_tibble()  # nolint
} else {
  data_raw <-
...
```{r remove-dead}
n_dead <- sum(!data_cor$LifeStatus %in% c(TRUE, NA, "Normal", "Recruit"))
if (n_dead > 0) {
  text <- paste0(
    n_dead, " (", round(n_dead / nrow(data_cor) * 100, 2), "%)",
    " measurements with Diameter > min DBH were considered as dead, and ",
    "removed from the analysis. Their LifeStatus values were: ",
    paste(unique(
      setdiff(data_cor$LifeStatus, c(TRUE, NA, "Normal", "Recruit"))
    ), collapse = ", "),
    "."
  )
} else {
...
data_cor <- data_cor |>
  subset(LifeStatus %in% c(TRUE, NA, "Normal", "Recruit")) # to be checked
```
...
data_aggr |>
  ...
  write.csv(paste0(
    ...
    paste(tolower(unique(data_aggr$Site)), collapse = "_"),
    ".csv"
  ))
```
