...
# Calculate diversity indices per plot and year
  ...
  species_count <- data_cor %>%
    group_by(Plot, Year, ScientificName) %>%
  ...
  results_long <- neutral_div %>%
    pivot_longer(cols = c(species_richness, shannon_entropy, simpson_index, evenness), names_to = "index", values_to = "value")

  # Plot temporal trajectories of different indices across plots
  div_plot <- ggplot(results_long, aes(x = Year, y = value, color = as.factor(Plot))) + 
    geom_line() +
    facet_wrap(~index, scales = "free_y") +
    labs(title = "Diversity Indices Across Plots and Years", x = "Year", y = "Index Value", color = "Plot" ) +
    theme_minimal()
