...
data_raw <-
  ...
  mutate(Site = as.character(Site)) |>
  rename(site_raw = Site) |>
  # todo solve issue with site matching
...
if (params$site == "suas") {
  data_cor <- data_cor |>
    separate(ScientificName, c("genus_cleaned", "species_cleaned"),
      remove = FALSE
    ) |>
    mutate(scientific_cleaned = paste(genus_cleaned, species_cleaned))
}
```

```{r temp-complete-species}
if ("Uppangala" %in% data_raw$Site) {
  data_species_complete <- read_tsv("data/derived_data/taxonomy_v5.tsv") |>
...
if (any(data_cor$Site == "BAFOG")) {
  ...
  text <- paste(
    text,
    "In BAFOG, we removed the census of 2017 for plot VI (partial measurement)."
  )
}
```
