...
data_cor <- data_cor |>
  group_by(IdTree) |>
  mutate(
    diameter_growth = c(NA, diff(diameter_cor)) / c(NA, diff(Year)),
...
```{r}
gr_range <- quantile(data_cor$diameter_growth, c(0.01, 0.99), na.rm = TRUE)
subset(
  data_cor,
  diameter_growth >= gr_range[1] & diameter_growth <= gr_range[2]
) |>
  ggplot(aes(x = diameter_growth)) +
...
average_growth <- data_cor |>
  subset(diameter_growth >= gr_range[1] & diameter_growth <= gr_range[2]) |>
  with(mean(diameter_growth, na.rm = TRUE))

data_cor <- data_cor |>
  mutate(diameter_growth = ifelse(
    !is.na(diameter_growth) &
      (diameter_growth < gr_range[1] | diameter_growth > gr_range[2]),
    average_growth,
    diameter_growth
  ))
