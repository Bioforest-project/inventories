...
single_site <- TRUE
if (single_site) {
  data_raw <- read.csv("data/raw_data/harmonized_data_moju_v1.csv")
  # data_raw <- read.csv("data/raw_data/harmonized_data_kibale_v1.csv") # nolint
} else {
  data_raw <-
    list.files("data/raw_data/", pattern = "paracou", full.names = TRUE) |>
    lapply(read.csv) |>
...
div_plot <-
  ggplot(
    results_long,
    aes(x = Year, y = value, color = as.factor(Plot))
  ) +
  geom_line() +
...
perc_undet <- data_cor |>
  group_by(Site, Plot, Year) |>
  summarise(p_undet = sum(is.na(scientific_cleaned)) / n())

data_aggr <- data_aggr |>
  merge(perc_undet)

data_aggr |>
  ggplot(aes(x = Year, y = p_undet * 100, colour = factor(Plot))) +
  geom_line() +
