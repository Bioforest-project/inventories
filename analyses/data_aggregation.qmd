...
data_cor <- data_cor |>
  ...
  subset(!(LifeForm %in% c("Palm", "Tree fan", "Death", "Vine"))) |>
...
neutral_div_sp <- species_count %>%
  group_by(Site, Plot, Year) %>%
  summarise(
    species_richness_sp = n_distinct(scientific),
    shannon_entropy_sp = as.numeric(Shannon(count)),
    simpson_index_sp = 1 - as.numeric(Simpson(count)),
    evenness_sp = as.numeric(Shannon(count)) / log(n_distinct(scientific))
  ) %>%
  pivot_longer(
    cols = contains("_sp"),
    names_to = "variable",
...
neutral_div_gen <- data_cor %>%
  ...
  group_by(Site, Plot, Year) %>%
  summarise(
    species_richness_gen = n_distinct(genus),
    shannon_entropy_gen = as.numeric(Shannon(count)),
    simpson_index_gen = 1 - as.numeric(Simpson(count)),
    evenness_gen = as.numeric(Shannon(count)) / log(n_distinct(genus))
  ) %>%
  pivot_longer(
    cols = contains("_gen"),
    names_to = "variable",
...
data_aggr <- data_cor |>
  ...
  group_by(Site, Plot, Year, variable = paste0(trait, "_ba")) |>
...
var_names <- c(
  "LA_ba" = "Leaf area (mm2)",
  "N_ba" = "Leaf nitrogen content per leaf dry mass (mg g-1)",
  "SLA_ba" = "Specific leaf area (mm2 mg-1)",
  "WD_ba" = "Stem specific wood density (g cm-3)",
  "hmax_ba" = "Asymptotic height (m)"
)

data_aggr |>
  subset(grepl("_ba$", variable)) |>
  ggplot(aes(
...
```{r plot-agb, fig.height=4, fig.width=12}
#| warning: false
var_names <- c(
...
data_aggr <- data_cor |>
  ...
  mutate(ba_gr_yr = ba_growth / (Year - prev_census)) |>
  group_by(Site, Plot, Year) |>
