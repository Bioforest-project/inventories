...
data_raw <-
  ...
  mutate(Site, gsub(pattern = " ", replacement = "", Site)) |>
  mutate(site = gsub(pattern = "_", replacement = "", Site))
```
...
plot_information <- read.csv("data/raw_data/bioforest-plot-information.csv") |>
  ...
  mutate(site = ifelse(site == "Montagne_tortue", "montagnetortue", site)) |>
  mutate(site = tolower(site)) |>
  mutate(site, gsub(pattern = " ", replacement = "", site)) |>
  mutate(site = gsub(pattern = "_", replacement = "", site)) |>
  subset(tolower(site) %in% tolower(unique(data_cor$Site)))
```

```{r add-missing-plot-coordinates}
# temporarily add plot information when missing online
plot_information <- plot_information |>
  #kibale coordinates added
  #mutate(
  # longitude = ifelse(site == "Kibale", 30.425, longitude),
  #  latitude = ifelse(site == "Kibale", 0.45, latitude)
  #) |>
  mutate(
    longitude_cor = ifelse(site == "km67", replace_na(longitude, -54.930322), longitude),
    latitude_cor = ifelse(site == "km67", replace_na(latitude, -54.930322), latitude)
  ) |>
...
if (nrow(data_cor) == 0) {
  stop(paste0(
    ...
    "on the online document: https://docs.google.com/spreadsheets/d/1fq2owxMBLBwwibcdw2uQQFxnIhsMbaH4Qcj_xUwVvSQ/edit?usp=drive_link"
  ))
}
```
...
```{r clean-species-information}
# clean columns with taxonomic information (vernacular and scientific)
if (!"VernName" %in% colnames(data_cor)) data_cor$VernName <- NA
if (!"ScientificName" %in% colnames(data_cor)) data_cor$ScientificName <- NA

data_cor <- data_cor |>
  mutate(
    VernName = iconv(VernName, "UTF-8", "ASCII//TRANSLIT"),
    ScientificName = iconv(ScientificName, "UTF-8", "ASCII//TRANSLIT")
  ) |>
  separate(VernName,
    sep = "\\[|\\]", c("vernname_1", "vernname_2"),
    extra = "drop", fill = "left", remove = FALSE
  ) |>
  mutate(
    ScientificName = ifelse(
      is.na(ScientificName) | grepl("\\[", ScientificName),
      vernname_2, ScientificName
    ),
    vernname_1 = NULL,
    vernname_2 = NULL
  )

# add genus and vername columns if missing
if (!"Genus" %in% colnames(data_cor)) {
  data_cor <- data_cor |>
    separate(ScientificName, "Genus", extra = "drop", remove = FALSE)
}
```

```{r add-clean-taxonomy}
# add clean species information
data_species <-
  read.delim("data/derived_data/taxonomy_v2.tsv") |>
  select(contains("cleaned") | contains("genus") | contains("species")) |>
  unique()
...
data_cor <- data_cor |>
  ...
  text <- paste(
    ...
    "(such as trees, lianas, palms), so we assume that all individuals in your dataset are trees.",
...
```{r taper-correction-bis}
# This is to test the correction made when using the new equation for taper correction (test GD)
# here I take the results of Model 1
# THIS IS NOT GOOD
#| eval: false
taper <- function(diam, hom) {
  # default hom (no correction) when there is no reliable information
  hom[is.na(hom) | hom <= 0 | hom > 20] <- 1.3
  b <- exp(-0.3091 -0.6147 * log(diam) -0.6071 * log(hom))
  return(diam * exp(b * (hom - 1.3)))
}
...
```{r taper-correction-ter}
# This is to test the model of the 2021 paper (test GD)
#| eval: false
taper <- function(diam, hom) {
  # default hom (no correction) when there is no reliable information
  hom[is.na(hom) | hom <= 0 | hom > 20] <- 1.3
  b <-  0.156  -0.023 * log(diam) -0.021 * log(hom)
  return(diam * exp(b * (hom - 1.3)))
}
...
# data_cor |> filter(diameter_cor < Diameter) |> summarise(range(HOM))
# the case when we have a lower corrected values are for cases when the HOM < 1.3

# the correction seems ok for the low HOM but bad for the high ones...
# in the 2021 paper, the range of HOM is 1.5 to 7.17
    subset(data_cor, HOM != 1.3 & HOM < 7.2) |>
      ggplot(aes(x = Diameter, y = diameter_cor, col = HOM)) +
      geom_abline(intercept = 0, slope = 1, lty = 2) +
      geom_point() +
      labs(x = "Original diameter (cm)", y = "Taper-corrected diameter (cm)") +
...
<!-- There are HOM below 1.3m.... We need to decide what to do with them -->
...
if (sum(is.na(data_cor$Diameter)) > 0) {
  ...
  illustration <- sample(
    ...
    min(sum(is.na(data_cor$Diameter)), 12)
  )
...
neutral_div_sp <- data_cor %>%
  subset(!is.na(species_corrected)) %>%
  group_by(Site, Plot, Year,
    scientific = paste(genus_corrected, species_corrected)
  ) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(Site, Plot, Year) %>%
  summarise(
    richness_sp = Richness(count),
    diversity_q0_sp = Diversity(count, q = 0, SampleCoverage = 0.8),
    diversity_q1_sp = Diversity(count, q = 1, SampleCoverage = 0.8),
    diversity_q2_sp = Diversity(count, q = 2, SampleCoverage = 0.8),
    evenness_sp = Shannon(count, SampleCoverage = 0.8) /
      log(n_distinct(scientific)),
...
neutral_div_gen <- data_cor %>%
  subset(!is.na(genus_corrected)) %>%
  group_by(Site, Plot, Year, genus = genus_corrected) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(Site, Plot, Year) %>%
  summarise(
    richness_gen = Richness(count),
    diversity_q0_gen = Diversity(count, q = 0, SampleCoverage = 0.9),
    diversity_q1_gen = Diversity(count, q = 1, SampleCoverage = 0.9),
    diversity_q2_gen = Diversity(count, q = 2, SampleCoverage = 0.9),
    evenness_gen = Shannon(count, SampleCoverage = 0.9) /
...
data_aggr <- data_cor |>
  ...
  merge(
    read.delim("data/derived_data/traits_v2.tsv"),
    by = "scientific_cleaned"
  ) |>
  group_by(Site, Plot, Year, variable = paste0(trait, "_cwm_ba")) |>
  reframe(value = weighted.mean(trait_value_imputed, ba)) |>
  rbind(data_aggr)
```
...
data_aggr <- data_cor |>
  ...
  reframe(value = mean(trait_value_imputed, na.rm = TRUE)) |>
  rbind(data_aggr)
```
...
data_aggr <- data_cor |>
  ...
  reframe(value = var(trait_value_imputed)) |>
  rbind(data_aggr)
```
...
```{r functional-div}
matrix_trait <- read.delim("data/derived_data/traits_v2.tsv") |>
...
```{r add-wd}
# add wood density
data_cor <- read.delim("data/derived_data/traits_v2.tsv") |>
  subset(trait == "WD") |>
  pivot_wider(
    id_cols = "scientific_cleaned",
    names_from = trait,
