...
-   asked for files required to run this quarto script, namely the taxonomic corrections and functional traits 

-   made sure all input files are in the same folder

-   made sure that your file is named harmonized_data_site-name_versionname.csv

Note: You need to use the same character strings in your harmonized file name and in the "params" at the beginning of this file.
...
### Checking and removing missing (NA) and zeros in DBH values
...
The first step is to remove all the dead individuals if there is a column called "LifeStatus".
...
The second step is to remove any other life forms apart from trees, such as Lianas, palms, cacti etc.
...
We want to now ensure that indidivuals have the same sicentific name across censuses.
...
if (any(!is.na(species_groups$med_growth))) {
  species_groups |>
    ...
    group_by(sp_group) |>
    summarise(
      range = paste(paste(round(range(med_growth), 2), collapse = " - "),
                    "cm/yr"),
      species = paste(scientific_cleaned, collapse = ", ")
    ) |>
    select(range, species) |>
    t() |>
    knitr::kable(col.names = paste(
