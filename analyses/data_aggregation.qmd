...
The following plots do not have information on either their coordinates or their treatment and have been removed from the analysis: `r paste(subset(data_missing, is.na(longitude))$idplot, collapse = ", ")`. Please add their information to the [online document](https://docs.google.com/spreadsheets/d/1fq2owxMBLBwwibcdw2uQQFxnIhsMbaH4Qcj_xUwVvSQ/edit?usp=drive_link) or check that the site and plot names are consistent between your harmonized data and the online document.   
...
data_cor <- data_cor |>
  mutate(
    genus_raw = tstrsplit(ScientificName, " ")[[1]],
    species_raw = tstrsplit(ScientificName, " ")[[2]]
  )
...
data_cor <- data_cor |>
  merge(
    ...
    by = "scientific_cleaned", all.x = TRUE
  )
```
...
```{r}
data_aggr |>
  ggplot(aes(x = Year, y = agb, group = factor(Plot))) +
  geom_point() +
  geom_line() +
