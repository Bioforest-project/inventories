...
## Visualizing taxonomic identification resolution
...
We want to make sure that there are no duplicated measurements in the data. So we create a unique identifier for each stem by combining the name of the site, plot, subplot and tag/ID of the tree and stem. We then check that there are no duplicated measurements for the same stem ID in a given census. Here is the process we follow:
1. If there are fully identical rows present in the dataset, only the first row is kept and all others are removed.
...
if (length(unique(nb_stem_plot$n)) != 1) { # if there are some duplicates
  ...
  if (prev_nb_obs - dim(data_cor)[1] != 0) {
    ...
    text <- "We checked for true duplicates (fully identical rows), and there are none in this dataset. We will now move on to checking for multi-stemmed trees."
  }
...
if (any(data_nstem$nstem != 1)) {
  ...
  if (nrow(multi_stemmed) <= 10) {
    ...
    text <- paste(
      ...
      "As the number is too high, we are unable to print all the stem names.","Please check your data or discuss with us if this is to be expected."
    )
  }
}
```
