...
if (dim(data_sps_dup)[1] == 0) {
  text <- "Every tree has the same name across all censuses, you can proceed to the next step"
} else {
  text <- c("There are still individuals to be check for their identification. They have a different species name in different censuses. Please check the following table to identify the individuals that are not consistently named:", data_sps_dup)
}
...
col_tree <-
  !grepl("census|year|month|day|date|status|code|diam|hom|pom|circ|dbh",
         tolower(colnames(data_raw)))
missing_censuses <- merge(missing_censuses,
                          unique(data_raw[, col_tree]), by = "IdTree")
...
if (sum(is.na(data_raw$Diameter)) > 0) {
  text <- paste(sum(is.na(data_raw$Diameter)),
