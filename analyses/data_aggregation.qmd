...
This document will help you visualize various aspects of your data, understand the applied data correction steps, and generate an aggregated dataset. This aggregated data set (depending on the variables available) will be used for analysis by various working groups in the Bioforest project. TIt also facilitates feedback from Site PIs regarding any necessary changes to corrections and aggregations. WWe encourage every Site PI to carefully review this document and provide feedback.
...
-   Harmonized your **raw** dataset using the [DataHarmonization app](https://github.com/Alliance-for-Tropical-Forest-Science/DataHarmonization); or sent the **raw** dataset to the data handlers;
...
**If you are running the quarto script:**
...
read_harmonized <- function(file) {
  ...
  read_csv(file,
    ...
    # remove duplicated genus from scientific name
    mutate(ScientificName = ifelse(
      str_count(ScientificName, Genus) == 2,
...
plot_information <- plot_information |>
  mutate(plot = ifelse(site == "Sao Nicolau",
    paste(plot, Treatment, sep = "-"), plot
  ))

# add plot-level information to the table
data_cor <- data_cor |>
  merge(plot_information,
    by.x = c("Site", "Plot"), by.y = c("site", "plot"), all.x = TRUE
  )
...
text <- ifelse(
  nchar(missing) > 0,
  paste0(
    "The following plots in the dataset have missing coordinates and have
    been removed from",
...
The dataset has `r dim(data_cor)[1]` observations, belonging to `r length(unique(data_cor$IdStem))` stems.

## Summary of plots and censuses

The following table shows the number of censuses per plots, the first and last year of census, and the number of individuals measured in each census. If you feel like the information is not accurate, please let us know the precise coordinates if the information is available.
...
## Visualization of plots and censuses
...
map <- ggplot(data_cor, aes(x = longitude, y = latitude, 
                            color = as.factor(Plot))) +
  geom_point(alpha = 0.6, show.legend = FALSE) +
  # facet_wrap(~ Year) +
  labs(
    title = "Map of Plots by Census Year",
    x = "Longitude",
    y = "Latitude",
    color = "Plot ID"
  ) +
...
plots <- ggplot(data_cor, aes(x = Year, y = Plot, color = as.factor(Plot))) +
  geom_point(alpha = 0.6, show.legend = FALSE) +
  scale_y_discrete(breaks = c(min(data_cor$Plot), median(data_cor$Plot), max(data_cor$Plot))) +
  # facet_wrap(~ Year) +
  labs(
...
if (params$site %in% c("paracou", "montagne-tortue", "bafog", "manare")) {
  vern <- data_cor |>
    select(vernacular_raw, scientific_cleaned) |>
    drop_na() |>
      subset(vernacular_raw != "-" &
      !str_detect(substr(vernacular_raw, 1, 1), "[[:upper:]]")) |>
    group_by(vernacular_raw) |>
...
if (anyNA(data_cor$scientific_cleaned)) {
  ...
  text <- paste(
    ...
    nb_na_sp_tree,
    "trees.",
    "The observations with NAs in the scientific names are distributed across",
    "the plots and censuses as shown in the figure below"
  )
} else {
...
plot_data$name <- recode(plot_data$name,
  pundet = "Undetermined",
  pgenus = "Genus Only",
  pspecies = "Species"
)

# Count number of unique plots
n_plots <- length(unique(plot_data$Plot))

# Set number of columns in facet_wrap
ncol_facets <- 4

# Calculate number of rows (ceiling to account for remainder)
nrow_facets <- ceiling(n_plots / ncol_facets)

# Set height per row (e.g., 3 inches per row)
height_per_row <- 3

# Final plot height
plot_height <- nrow_facets * height_per_row

# Create plot
ggplot(plot_data, aes(Year, value, fill = name)) +
  geom_col() +
  facet_wrap(~Plot, scales = "free_y", ncol = ncol_facets) +
  scale_y_continuous(breaks = c(0, 0.5, 1)) +
  labs(fill = "ID level", title = "Proportion of taxonomic ID resolution") +
  theme_minimal() +
  theme(legend.position = "bottom") +
  theme(axis.text.x=element_text(size=rel(0.7)),axis.text.y=element_text(size=rel(0.7)), strip.text = element_text(size=rel(0.7)))
```
...
We do not allow any NA values in the DBH columns. If this dataset has any NAs or zeros in the Diameter column, we remove those values before proceeding. If there are either NAs or zeros, we will list the numbers below in a table for you to confirm if the number seems realistic.
...
if (sum(data_cor$Diameter == 0, na.rm = TRUE) > 0) {
  ...
  text <- paste("There are",
    ...
    perc_zero_dbh,
    "% of the observations.",
    "The zeros are distributed across the plots",
    "and censuses as follow:",
    sep = " "
  )
} else {
  text <- "There are no zeros in the diameters in the data."
}
```
...
na_values <- if (anyNA(data_cor$Diameter)) {
  data_cor %>%
    filter(is.na(Diameter)) %>%
    count(Site, Plot, Year)
}

zero_values <- if (sum(data_cor$Diameter == 0, na.rm = TRUE) > 0) {
  data_cor %>%
    filter(data_cor$Diameter == 0, na.rm = TRUE) %>%
    count(Site, Plot, Year)
}


if (sum(zero_values) > 0 | (sum(na_values) > 0)) {
  merge(na_values, zero_values, by = c("Site", "Plot", "Year"), all = TRUE) |>
    knitr::kable()
}
```
...
If there were NAs or zeros present in the dataset, we remove them. The DBH values in this dataset are thus NA and zero free.
...
if (length(unique(data_cor$MinDBH)) == 1) {
  text <- paste(
    "In this dataset, there is a single DBH cutoff set to",
...
if (any(data_cor$Site == "Iwokrama")) {
  data_cor <- data_cor |>
    rename(IdMeas = IdTree) |>
    separate_wider_delim(IdMeas, "_", names = "IdTree", too_many = "drop") |>
    mutate(IdStem = paste(Site, Plot, Subplot, IdTree, sep = "_"))
  text <- paste(
...
if (all(length(unique(nb_stem_plot$n)) == 1, unique(nb_stem_plot$n) == 1)) {
  text <- "There is no duplicated stem identifiers within a same census in this
  site, so we move on to checking for multi-stemmed trees."
} else {
...
if (length(unique(nb_stem_plot$n)) != 1) { # if there are some duplicates
  ...
  if (prev_nb_obs - dim(data_cor)[1] != 0) {
    ...
    text <- "We checked for true duplicates (fully identical rows), and there
    are none in this dataset."
  }
  nb_stem_plot_bis <- data_cor %>%
    count(IdStem, IdCensus)
} else { # if no duplicated rows
  text <- "We checked for true duplicates (fully identical rows), and there are
...
We then check for duplicates within the same census.
...
The first step is to remove all the dead individuals if there is a column called "LifeStatus". We keep the rows with the character strings: TRUE, NA, "Normal", "Recruit", and the numbers 1,2,3,4. If your dataset has any other code to represent live/dead trees, please let us know.
...
if ("LifeStatus" %in% colnames(data_cor)) {
  ...
  text <- paste(
    "In this dataset, there was no column 'LifeStatus' with information about life status",
...
if ("LifeForm" %in% colnames(data_cor)) {
  data_cor <- data_cor |>
    ...
    subset(!(tolower(LifeForm) %in% c("palm","palms", "tree fan", "death", "vine", "vines", "lianas"))) |>
  ...
  text <- paste(
    "We only keep the life form tree in the column titled LifeForm."
  )
} else {
  text <- paste(
    "In this dataset, there was no column 'LifeForm' with information about life forms",
...
if (all(data_cor$SpName_codecor == "no_cor")) { # no duplicated names
  ...
  if (nb_filled != 0) {
    text1 <- paste(
      "There were",
      nb_filled,
      "observations with NA in the Scientific names,
                   but for which other observations of the same trees
                   have a unique scientific name.
...
Here we defined 'similar' trees by first grouping them by species and size (DBH), assuming that these two factors are among the main drivers of tree growth. We ranked species by median DBH growth and divided them into four groups of increasing median DBH growth, with similar numbers of individual measurements per group.

```{r sp-groups}
#| message: false
#| warning: false
species_groups <- data_cor %>%
  filter(!is.na(scientific_cleaned)) %>%
  # get median growth (excluding negative values) per species
  ...
  mutate(sp_group = ntile(med_growth, 4)) %>%
  group_by(scientific_cleaned, med_growth) %>%
  # summarize back to species level
  summarise(sp_group = first(sp_group), .groups = "drop")
```

```{r sp-group-visualisation}
#| message: false
#| warning: false
if (any(!is.na(species_groups$med_growth))) {
  species_groups |>
    subset(!is.na(sp_group) & !is.na(scientific_cleaned)) |>
    group_by(sp_group) |>
    summarise(
      range = paste(
      ...
      species = paste(scientific_cleaned, collapse = ", ")
    ) |>
    select(range, species) |>
    t() |>
    knitr::kable(
      col.names = paste(
        "Group", seq_len(length(unique(drop_na(species_groups)$sp_group)))
      ),
      row.names = FALSE
    )
}
...
We then split each species group into 5 quantiles of DBH: this results in 20 groups of DBH growth measures based on both species-level growth rates and size (DBH).
...
#| warning: false
data_cor <- data_cor |>
  merge(species_groups, by = "scientific_cleaned") |>
  group_by(sp_group) |>
  mutate(diam_group = paste(sp_group, ntile(diameter_cor, 5), sep = "_")) |>
  # lower diameter growth threshold is the same for all species and sizes
  ungroup() |>
  mutate(dgrowth_lower = quantile(diam_growth, 0.005, na.rm = TRUE)) |>
  # upper diameter growth threshold defined by species and size
  group_by(diam_group) |>
  mutate(dgrowth_upper = quantile(diam_growth, 0.995, na.rm = TRUE)) |>
  # define outlier values
  mutate(outlier = !is.na(diam_growth) &
    (diam_growth < dgrowth_lower | diam_growth > dgrowth_upper)) |>
...
if (sum(data_cor$outlier) > 0) {
  data_thresh <- data_cor |>
    subset(!is.na(sp_group)) |>
    group_by(sp_group, diam_group) |>
    summarise(
  ...
  data_cor |>
    ...
    ylim(-5, 10) +
    facet_wrap(~ paste("Species group", sp_group)) +
    scale_x_log10() +
    theme_classic()
}
```
...
To evaluate taxonomic diversity, we use functions from the entropart package in R (Marcon et al., 2020). We estimate all these indices at both species and genus level.
...
Order q = 0: Species richness, which counts all species equally, regardless of their abundance.

### Shannon Diversity

Order q = 1: Shannon diversity, calculated as the exponential of Shannon entropy, which gives proportional weight to species based on their relative abundances. This index balances sensitivity to both common and rare species.

### Simpson Diversity

Order q = 2: Simpson diversity, calculated as the inverse of the Simpson index, which emphasizes dominant species and is less sensitive to rare taxa.

### Evenness

Evenness: Evenness reflects how uniformly individuals are distributed among the observed species. It was derived by dividing the observed diversity (for a given value of q) by the theoretical maximum diversity for the same number of species. Values closer to 1 indicate communities where individuals are more evenly distributed across species, while lower values indicate dominance by a few species.

### Coverage

Coverage: Sample coverage estimates the proportion of individuals in a community that belong to species already detected in the sample. It provides an indication of sampling completeness, with values approaching 1 suggesting that most of the community has been captured and that further sampling is unlikely to yield many additional species.
...
To evaluate the functional structure of plant communities, we assessed both functional composition and functional diversity using a set of ecologically meaningful plant traits. These metrics provide insight into how species differ in their ecological roles and how trait distributions shape community assembly.
...
-   Leaf area: Associated with light capture and leaf construction cost

-   Leaf nitrogen content per leaf dry mass: Reflects photosynthetic capacity and nutrient-use strategy.

-   Specific leaf area (SLA): Indicates growth rate and resource acquisition.

-   Stem specific wood density: Related to mechanical strength, water transport efficiency, and survival strategies.

-   Asymptotic height: A proxy for maximum potential plant height.
...
var_names <- c(
  ...
  "N_cwm_ba" = "Leaf nitrogen content \n per leaf dry mass (mg g-1)",
...
data_aggr |>
  ...
  geom_point(show.legend = FALSE) +
  labs(
    title = "Functional Composition Across Plots and Years",
    x = "Year", y = "Index Value", color = "Plot"
  ) +
  theme_minimal() +
...
data_aggr |>
  ...
  geom_line(show.legend = FALSE) +
  geom_point(show.legend = FALSE) +
  labs(title = "Functional Variance Across Plots and Years",
    x = "Year", y = "Index Value", color = "Plot") +
  theme_minimal() +
  facet_wrap(~variable, scales = "free_y", labeller = as_labeller(var_names))
```
...
To describe the variation and structure of traits within communities, we we use the above functional traits `r traits_used` to calculate multiple complementary functional diversity indices. These indices capture different dimensions of diversity within trait space, based on species’ positions and abundances:

Functional Richness (scaled): Measures the volume of trait space occupied by the community, standardized for comparability. It reflects the range of functional strategies present, regardless of species abundance.

Functional Divergence: Indicates how abundance is distributed toward the edges of trait space. Higher values suggest that dominant species are functionally distinct from the community centroid, promoting niche differentiation.

Functional Evenness: Quantifies the regularity of species spacing and abundance distribution within trait space. High evenness implies that trait values are uniformly distributed and that resource use is balanced.

Functional Dispersion: Represents the mean distance of individual species to the community’s trait centroid, weighted by abundance. It accounts for both the spread and distribution of traits and is sensitive to both richness and dominance.
...
data_aggr |>
  ...
  geom_point(show.legend = FALSE) +
  labs(
    title = "Functional Indices Across Plots and Years",
    x = "Year", y = "Index Value", color = "Plot"
  ) +
  theme_minimal() +
  facet_wrap(~variable, scales = "free_y", labeller = as_labeller(var_names))
...
We characterized the size structure of the tree community using mean, minimum, mean and median of the DBH, reflecting the general size of individuals.
...
data_aggr |>
  subset(variable %in% c("gini", "dbh_median", "dbh_mean", "dbh_min", "dbh_max")
         ) |>
  ggplot(aes(x = Year, y = value, color = factor(Plot))) +
  geom_line(show.legend = FALSE) +
  geom_point(show.legend = FALSE) +
  labs(
    title = "Structural composition Across Plots and Years",
    x = "Year", y = "Index Value", color = "Plot"
  ) +
  theme_minimal() +
  facet_wrap(~variable, scales = "free_y", labeller = as_labeller(var_names))
...
data_aggr |>
  ...
  geom_point(show.legend = FALSE) +
  labs(
    title = "Demographic rates Across Plots and Years",
    x = "Year", y = "Index Value", color = "Plot"
  ) +theme_minimal() +
  facet_wrap(~variable, scales = "free_y", labeller = as_labeller(var_names))
```
...
data_aggr |>
  ...
  geom_point(show.legend = FALSE) +
  labs(
    title = "Structural Diversity Across Plots and Years",
    x = "Year", y = "Index Value", color = "Plot"
  ) +
  theme_minimal() +
  facet_wrap(~variable, scales = "free_y", labeller = as_labeller(var_names))
...
data_aggr |>
  write.csv(paste0(
    ...
    paste(gsub(" ", "-", tolower(unique(data_aggr$Site))), collapse = "_"),
    ".csv"
  ), row.names = FALSE)
```
