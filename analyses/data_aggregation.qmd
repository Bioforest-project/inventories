...
if (length(unique(data_cor$IdStem)) == length(unique(data_cor$IdTree))) {
  ...
  if (length(unique(data_cor$IdStem)) > length(unique(data_cor$IdTree))) {
    text <- "There are more stem identifiers than tree identifiers (as given in the original dataset), suggesting that there are some multi-stemmed trees." # nolint
  } else {
    text <- "There are less stem identifiers than tree identifiers (as given in the original dataset), which suggest a problem of duplicated stem identifiers." # nolint
  }
}
```
...
```{r}
data_cor$DiameterCor <- data_cor$Diameter
```
...
if (any(colnames(data_cor) == "HOM")) {
  ...
  data_cor$DiameterCor <- taper(data_cor$DiameterCor, data_cor$HOM)
...
missing_censuses <- data_cor |>
  ...
  # add DBH information
  merge(
    data_cor[, c("IdTree", "Year", "DiameterCor")],
    by = c("IdTree", "Year"),
    all = TRUE
  ) |>
