...
if (any(!is.na(species_groups$med_growth))) {
  species_groups |> 
    ...
    group_by(sp_group) |> 
    summarise(range = paste(round(range(med_growth), 2), collapse = " - "), 
              species = paste(scientific_cleaned, collapse = ", ")) |> 
    select(range,	species) |> 
    knitr::kable(col.names = c("Range of median diameter growth (cm)", "Species"))
}
```

#### Create diameter quantiles per species group
...
if (sum(data_cor$outlier) > 0) {
  text <- paste("This graph shows all outlier DBH growth values (in cm/yr),", 
                ...
                "values are replaced (blue), as a function of tree DBH.")
  ...
  data_cor |>
    ...
    ylim(-5, 10) +
    facet_wrap(~sp_group, labeller = label_both) +
    scale_x_log10() +
    theme_classic()
...
if (sum(data_cor$outlier) > 0) {
  id_cor <-
    ...
    subset(outlier) |> 
    group_by(sp_group) |> 
    summarise(id = sample(unique(IdStem), min(1, length(unique(IdStem)))))
