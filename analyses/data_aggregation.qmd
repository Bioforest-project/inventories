...
data_cor |>
  ggplot(aes(
    x = diameter_cor, y = diam_growth,
    color = (diam_growth >= dgrowth_lower & diam_growth <= dgrowth_upper)
  )) +
  geom_point() +
  labs(x = "Diameter (cm)", y = "Diameter growth (cm/yr)", col = "outlier?") +
  scale_x_log10() +
  theme_classic()
```

This graph shows all DBH growth values (in cm/yr) ... 
