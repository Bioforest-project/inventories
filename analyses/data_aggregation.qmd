...
path_metadata <- "data/raw_data/bioforest-plot-information"
if (!file.exists(paste0(path_metadata, ".csv"))) {
...
if (!"e" %in% colnames(read.csv(paste0(path_metadata, ".csv")))) {
  read.csv(paste0(path_metadata, ".csv")) |>
    mutate(e = computeE(cbind(longitude, latitude))) |>
    write.csv(paste0(path_metadata, ".csv"), row.names = FALSE)
}
```

```{r get-plot-level-data}
plot_information <- read.csv(paste0(path_metadata, ".csv")) |>
  mutate(site = as.character(site)) |>
  rename(site_raw = site) |>
  left_join(sites) |>
  select(-site_raw) |>
...
if (params$site %in% c("paracou", "montagne-tortue", "bafog", "manare")) {
  vern <- data_cor |>
    ...
    subset(vernacular_raw != "-" &
      !str_detect(substr(vernacular_raw, 1, 1), "[[:upper:]]")) |> # nolint
...
if ("LifeStatus" %in% colnames(data_cor)) {
  ...
  text <- paste(
    "In this dataset, there was no column 'LifeStatus' with information about",
    "life status (dead/alive), so we considered all stems to be alive: please",
    "let us know if we should have used another column."
  )
}
```
...
if ("LifeForm" %in% colnames(data_cor)) {
  data_cor <- data_cor |>
    ...
    subset(!(tolower(LifeForm) %in% c(
      "palm", "palms", "tree fan", "death",
      "vine", "vines", "lianas"
    ))) |>
...
data_aggr |>
  ...
  geom_point(show.legend = FALSE) +
  labs(
    title = "Functional Variance Across Plots and Years",
    x = "Year", y = "Index Value", color = "Plot"
  ) +
  theme_minimal() +
  facet_wrap(~variable, scales = "free_y", labeller = as_labeller(var_names))
...
data_aggr |>
  subset(variable %in% c(
    "gini", "dbh_median", "dbh_mean",
    "dbh_min", "dbh_max"
  )) |>
